...
const removeWatchersRecursively = async (_socket, _bodyHash, _i) => {
    try {
        if (_i === 100) return console.error(new Date().toISOString(), "watchers/removeWatchersRecursively/stoped");
        if (!_watchers[_socket.id] || !_watchers[_socket.id][_bodyHash]) throw new Error();
        ...
        return setTimeout(removeWatchersRecursively.bind(this, _socket, _bodyHash, _i + 1), 1000);
    }
};
...
module.exports = {
    setBySocketId: (_socket, _client, _db, _body, _callback) => {
        try {
            const changeStream = _db.collection(_body.collection).watch([], { fullDocument: "updateLookup" });
            changeStream.on("change", _callback);
            changeStream.on("error", error =>
                changeStream.close(() =>
                    _client.close(() => console.error(new Date().toISOString(), "watchers/setBySocketId/error", error))
                )
            );
    ...
    removeAllBySocketId: async _socket => {
        try {
            ...
            for (let i = 0; i < keys.length; i++) await removeWatchersRecursively(_socket, keys[i], 0);
